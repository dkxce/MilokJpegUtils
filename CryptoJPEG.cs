...
//
// ABOUT
// Milok Zbrozek Encrypt DATA and add 2 JPEG EXIF CSharp Script
// milokz@gmail.com
// Last Modified: 05.09.2019
//
// Use with CSCS.exe ( www.csscript.net )
//     cscs.exe /nl "<file_name>"
//
// Run from Notepad++ ( notepad-plus-plus.org )
//     cscs.exe /nl ""$(FULL_CURRENT_PATH)""
//     csc.exe /nl ""$(FULL_CURRENT_PATH)""
//     cscs.exe ""$(FULL_CURRENT_PATH)""
...
{
    public class CryptoEXIF
    {
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        public static string KEYWORD = @"0n14NaNzGtJtr07yb14dkxce8"; // keyword or password to crypt data    
        public static bool USECRYPT = true;

        static ulong ScriptVersion = 201909050001; // YYYYMMDDvvvv
        static string ExecutingScriptFileName = String.Empty;
        public static Exception LastError = null;

        public static string WriteDataToJPEG(string user_data_file, string jpeg_file, Dictionary<string, object> exif_data)
        {
            ExifInfo.MyBitConverter bc = new ExifInfo.MyBitConverter(false);
            FileInfo fi = new FileInfo(user_data_file);
            byte[] file_data = new byte[0];

            Random rnd = new Random();
            byte[] KEYSALT = new byte[16];
            for (int i = 0; i < KEYSALT.Length; i++)
                KEYSALT[i] = (byte)rnd.Next(0, 255);


            using (Stream user_file = USECRYPT
                ? (Stream)(new MemoryStream(Crypto.Encrypt(user_data_file, KEYWORD, KEYSALT)))
                : (Stream)(new FileStream(user_data_file, FileMode.Open, FileAccess.Read)))
            {
                using (MemoryStream mem_compressed = new MemoryStream())
                {
                    mem_compressed.WriteByte(USECRYPT ? (byte)'C' : (byte)'U'); // crypted or uncrypted
                    mem_compressed.Write(bc.GetBytes((int)user_file.Length), 0, sizeof(int)); // size of crypted file
                    mem_compressed.Write(bc.GetBytes((int)fi.Length), 0, sizeof(int)); // size of original file
                    using (System.IO.Compression.GZipStream compressionStream = new System.IO.Compression.GZipStream(mem_compressed, System.IO.Compression.CompressionMode.Compress))
                    {
                        byte[] buffer = new byte[ushort.MaxValue];
                        int len = 0;
                        while ((len = user_file.Read(buffer, 0, buffer.Length)) > 0)
                            compressionStream.Write(buffer, 0, len);
                    };
                    file_data = mem_compressed.ToArray();
                };
            };
            if ((file_data.Length == 0) || (file_data.Length > 40000 /* 40K */))
            {
                LastError = new IOException("File is too Big (>40KB)");
                return "";
            };

            string jpeg_out = "";
            if (String.IsNullOrEmpty(fi.Extension))
                jpeg_out = Path.GetDirectoryName(user_data_file) + @"\" + fi.Name + ".jpg";
            else
                jpeg_out = Path.GetDirectoryName(user_data_file) + @"\" + fi.Name.Replace(fi.Extension, ".jpg");

            if (exif_data == null) exif_data = new Dictionary<string, object>();

            //// REQUIRED ////
            exif_data.Add("0x8880", file_data); // 0x8880 - File Data
            exif_data.Add("0x8881", System.Text.Encoding.GetEncoding(1251).GetBytes(fi.Name));   // 0x8881 - File Name
            exif_data.Add("0x8882", bc.GetBytes(DateTime.Now.ToOADate())); // 0x8882 - Creation JPEG File DateTime
            exif_data.Add("0x8883", bc.GetBytes(fi.CreationTime.ToOADate())); // 0x8883 - Creation Source File DateTime
            exif_data.Add("0x8884", bc.GetBytes(fi.LastWriteTime.ToOADate())); // 0x8884 - Modified Source File DateTime
            exif_data.Add("0x8885", System.Text.Encoding.GetEncoding(1251).GetBytes(System.Security.Principal.WindowsIdentity.GetCurrent().Name)); // 0x8885 - User Name
            exif_data.Add("0x888A", bc.GetBytes((ulong)ScriptVersion));   // 0x888A - ScriptVersion
            exif_data.Add("0x888B", KEYSALT);   // 0x888B - KEYSALT

            try
            {
                ExifInfo.ExifRewrite(jpeg_file, jpeg_out, exif_data);
            }
            catch (Exception ex)
            {
                LastError = ex;
        ...
        public static string ReadDataFromJPEG(string jpeg_in)
        {
            ExifInfo.MyBitConverter bc = new ExifInfo.MyBitConverter(false);

            ExifInfo exi = ExifInfo.ParseExifFile(jpeg_in);
            ExifInfo.IFD_Entry eFileData = exi[0x8880];
            ExifInfo.IFD_Entry eFileName = exi[0x8881];
            ExifInfo.IFD_Entry eJPEGCrtd = exi[0x8881];
            ExifInfo.IFD_Entry eFileCrtd = exi[0x8883];
            ExifInfo.IFD_Entry eFileMdfd = exi[0x8884];
            ExifInfo.IFD_Entry eFileUser = exi[0x8885];
            ExifInfo.IFD_Entry scriptVer = exi[0x888A];
            ExifInfo.IFD_Entry keySaltDT = exi[0x888B];

            ulong sv = 0;
            if (scriptVer != null) sv = (ulong)scriptVer.entry_value;
            if (sv == 0)
            {
                LastError = new IOException("Version Tag is not found");
                return "";
            };
            byte[] KEYSALT = new byte[0];
            if (keySaltDT == null)
            {
                LastError = new IOException("KeySalt not found");
                return "";
            }
            else
                KEYSALT = (byte[])keySaltDT.entry_value;

            if ((eFileData != null) && (eFileName != null) && (eFileCrtd != null) && (eFileMdfd != null))
            {
                byte[] file_data = (byte[])eFileData.entry_value;
                string file_name = eFileName.entry_value.ToString();
                DateTime file_created = (DateTime)eFileCrtd.entry_value;
                DateTime file_modified = (DateTime)eFileMdfd.entry_value;

                if (String.IsNullOrEmpty(file_name))
                {
                    LastError = new IOException("FileName Not Found");
                    return "";
                };

                if ((!String.IsNullOrEmpty(file_name)) && (file_data != null) && (file_data.Length != 0))
                {
                    string resfile = Path.GetDirectoryName(jpeg_in) + @"\" + file_name;

                    using (MemoryStream mem_compressed = new MemoryStream(file_data))
                    {
                        bool crypted = (char)file_data[0] == 'C' ? true : false;
                        using (Stream mem_decompressed = crypted
                            ? (Stream)(new MemoryStream())
                            : (Stream)(new FileStream(resfile, FileMode.Create, FileAccess.Write)))
                        {
                            int cryp_size = bc.ToInt32(file_data, 1);
                            int norm_size = bc.ToInt32(file_data, 5);
                            mem_compressed.Position = 9;
                            byte[] decompressedBytes = new byte[crypted ? cryp_size : norm_size];
                            using (System.IO.Compression.GZipStream decompressionStream = new System.IO.Compression.GZipStream(mem_compressed, System.IO.Compression.CompressionMode.Decompress))
                            {
                                byte[] buffer = new byte[ushort.MaxValue];
                                int len = 0;
                                try
                                {
                                    while ((len = decompressionStream.Read(buffer, 0, buffer.Length)) > 0)
                                        mem_decompressed.Write(buffer, 0, len);
                                }
                                catch (Exception ex)
                                {
                                    LastError = new IOException("Invalid ZIP Data", ex);
                                    return "";
                                };
                            };
                            if (crypted)
                            {
                                if (mem_decompressed.Length != cryp_size)
                                    throw new IOException("Invalid crypted data length");
                                mem_decompressed.Position = 0;
                                if (!Crypto.Decrypt(mem_decompressed, resfile, KEYWORD, KEYSALT))
                                    return "";
                            };
                        };
                    };
                    ...
                    fi.CreationTime = file_created;
                    fi.LastWriteTime = file_modified;
                    return resfile;
                };
            };
            LastError = new IOException("NO EXIF DATA");
        ...
        static string GetCurrentDir()
        {
            ...
            ExecutingScriptFileName = null;

            string[] cla = System.Environment.GetCommandLineArgs();
            if ((cla != null) && (cla.Length > 1))
                for (int i = 1; i < cla.Length; i++)
                    if (File.Exists(cla[i]))
                        ExecutingScriptFileName = cla[i];

            return String.IsNullOrEmpty(ExecutingScriptFileName) ? currentDir : Path.GetDirectoryName(ExecutingScriptFileName);
        }

        static string[] pattern_files(string[] file_list, string file_pattern)
        {
        ...
        public class ExifInfo
        {
            ...
            public static IFD_Entry[] ParseExifNrm(byte[] normallized_exif_data)
            {
                ...
                while (entry_offsets.Count > 0)
                {
                    ...
                    for (int i = 0; i < entNo; i++)
                    {
                        ...
                        {
                            LastError = new IOException("Read IFD Entry Error: " + ex.Message, ex);
                        };
                    };
            ...
            public string this[string key, string nullValue]
            {
                get
                {
                    ...
                    if (result == null)
                        ...
                        return result.entry_value.ToString().Trim('\0').Trim();
                }
            }
            ...
            public class IFD_Entry
            {
                ...
                {
                    object out_data = entry_value;
                    if (entry_value is byte[]) out_data = System.Text.Encoding.GetEncoding(1251).GetString((byte[])entry_value);
                    return String.Format("0x{0:X4} {1}: {2}", new object[] { entry_tag_number, entry_tag_name, out_data });
                }
                ...
                public string entry_text
                {
                    get
                    {
                        ...
                        if ((entry_value != null) && (entry_value is byte[]) && ((byte[])entry_value).Length > 0)
                        {
                            string res = "0x";
                            for (int i = 0; i < ((byte[])entry_value).Length; i++)
                                res += String.Format("{0:X2}",((byte[])entry_value)[i]);
                            return res;
                        };
                ...
                public static string TagNameByNumber(int number)
                {
                    switch (number)
                    {
                        ...
                        case 0x8769: return "ExifOffset"; // 0x8769
                        ...
                        case 0x8825: return "GPSInfo"; // 0x8825
                        ...
                        /* MY */
                        case 0x8880: return "CryptoJPEG.File.Data";
                        case 0x8881: return "CryptoJPEG.File.Name";
                        case 0x8882: return "CryptoJPEG.JPEG.CreationTime";
                        case 0x8883: return "CryptoJPEG.File.CreationTime";
                        case 0x8884: return "CryptoJPEG.File.ModifiedTime";
                        case 0x8885: return "CryptoJPEG.JPEG.UserCreated";
                        case 0x888A: return "CryptoJPEG.ScriptVertion";
                        case 0x888B: return "CryptoJPEG.KeySalt";
                        ////////
                        ...
                        case 0xA005: return "InteropOffset"; // 0xA005
                ...
                public IFD_Entry(int IFDNo, MyBitConverter bc, byte[] exif_data, int curr_offset)
                {
                    ...
                    string en = entry_tag_number.ToString("X");
                    ...
                    if ((entry_tag_number == 0x8881) || (entry_tag_number == 0x8885)) //CryptoJPEG.Tags
                    {
                        data_length_calculated = 1 * data_number_of_components;
                        data_offset_in_exif = data_length_calculated <= 4 ? (uint)curr_offset + 8 : (uint)data_value_or_offset;
                        byte[] user_data = new byte[data_length_calculated];
                    ...
                    if ((entry_tag_number == 0x8882) || (entry_tag_number == 0x8883) || (entry_tag_number == 0x8884)) //CryptoJPEG.Tags
                    {
                        data_length_calculated = 1 * data_number_of_components;
                        data_offset_in_exif = data_length_calculated <= 4 ? (uint)curr_offset + 8 : (uint)data_value_or_offset;
                        byte[] data = new byte[data_length_calculated];
                        Array.Copy(exif_data, (int)data_offset_in_exif, data, 0, data_length_calculated);
                        entry_value = DateTime.FromOADate((new MyBitConverter(false)).ToDouble(data, 0));
                        return;
                    }
                    if ((entry_tag_number == 0x888A)) //CryptoJPEG.Tags
                    {
                        data_length_calculated = 1 * data_number_of_components;
                        data_offset_in_exif = data_length_calculated <= 4 ? (uint)curr_offset + 8 : (uint)data_value_or_offset;
                        byte[] data = new byte[data_length_calculated];
                        Array.Copy(exif_data, (int)data_offset_in_exif, data, 0, data_length_calculated);
                        entry_value = (new MyBitConverter(false)).ToUInt64(data, 0);
                        return;
                    }
            ...
            public static void ExifRewrite(string source_file, string dest_file, Dictionary<string, object> new_exif)
            {
                ...
                if (new_exif != null) // WRITE NEW EXIF
                {
                    ...
                    {
                        if (data.Length > 0xFEFB) // No 0xFFFF marker
                        {
                            fr.Close();
                            fw.Close();
            ...
            private static byte[] CreateExif(Dictionary<string, object> new_exif)
            {
                ...
                if (ifd1_c > 0) // EX DATA
                {
                    ...
                    foreach (string key in new_exif.Keys)
                    {
                        if ((key.StartsWith("0x") && ((key.Length == 4) || (key.Length == 6))))
                        {
                            ushort tagID = ushort.Parse(key.Substring(2), System.Globalization.NumberStyles.HexNumber);
                            byte[] userdata = (byte[])new_exif[key];
                            data.AddRange(bc.GetBytes((ushort)tagID)); // type // 2
        ...
        public class Crypto
        {
            public static bool Encrypt(string inFile, string outFile, string keyword, byte[] keysalt)
            {
                bool isok = true;
                ...
                {
                    isok = false;
                    LastError = new IOException("Encrypt Error: " + ex.Message, ex);
                }
                finally
                {
                    if (aesAlg != null)
            ...
            public static byte[] Encrypt(string inFile, string keyword, byte[] keysalt)
            {
            ...
            public static bool Decrypt(string inFile, string outFile, string keyword, byte[] keysalt)
            {
                bool isok = true;
                ...
                {
                    isok = false;
                    LastError = new IOException("Invalid KEY/SALT", ex);
                }
                finally
                {
                    if (aesAlg != null)
            ...
            public static bool Decrypt(Stream cryptedData, string outFile, string keyword, byte[] keysalt)
            {
                bool isok = true;
                RijndaelManaged aesAlg = null;
                ...
                try
                {
                    ...
                    aesAlg.Key = key.GetBytes(aesAlg.KeySize / 8);
                    // Get the initialization vector from the encrypted stream
                    aesAlg.IV = ReadByteArray(cryptedData);
                    // Create a decrytor to perform the stream transform.
                    ICryptoTransform decryptor = aesAlg.CreateDecryptor(aesAlg.Key, aesAlg.IV);
                    using (CryptoStream csDecrypt = new CryptoStream(cryptedData, decryptor, CryptoStreamMode.Read))
                    {
                        FileStream file_out = null;
                        byte[] buffer = new byte[ushort.MaxValue];
                        {
                            int len = 0;
                            while ((len = csDecrypt.Read(buffer, 0, buffer.Length)) > 0)
                            {
                                if (file_out == null)
                                    file_out = new FileStream(outFile, FileMode.Create, FileAccess.Write);
                                file_out.Write(buffer, 0, len);
                            };
                        };
